...
| `016_gallery_consolidation.sql` | 📄 written, ⏳ apply manually | Gallery consolidation | **Required before testing the merged gallery.** Drops `businesses.gallery_images`. Replaces the unique `(business_id, section_key)` constraint on `gallery_items` with a non-unique index so services and gallery can have multiple images per slot. CHECK-constrains `section_key` to `'hero' \| 'story' \| 'services' \| 'gallery'`. |
| `017_ai_generation_events.sql` | 📄 written, ⏳ apply manually | Wizard progress streaming | **Required before testing the wizard's real-time progress.** Creates `ai_generation_events` table + RLS so the wizard subscribes to its own progress events via Supabase Realtime. **After running the SQL, also toggle Realtime ON for this table** in Studio → Database → Replication. Without that toggle, the wizard's substeps will fall back to its 8 s timer-based advancement — generation still works, but no live progress. |
| `018_photo_captions.sql` | 📄 written, ⏳ apply manually | AI art-direction captions | **Required before testing the AI photo captions in the Hub.** Adds `hero_photo_caption` and `story_photo_caption` to `website_customization`. The AI generator emits one-sentence captions describing the ideal hero and story photos; the Hub surfaces them under the upload slots so owners know what to shoot. |
| `019_business_booking_enabled.sql` | 📄 written, ⏳ apply manually | Phase E — opt-in booking | **Required before testing the registration booking toggle / profile toggle / sidebar Bookings gating.** Adds `booking_enabled BOOLEAN NOT NULL DEFAULT true` to `businesses`. Existing rows default to `true` so already-launched sites keep working. The flag gates wizard Step 2's booking-method question, sidebar's Bookings menu item, public-site BookingDrawer mounting, and the hero CTA's open-drawer vs open-WhatsApp dispatch. |
