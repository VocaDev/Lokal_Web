...
export default function BeautyLuxury({ business, services, hours }: {
  ...
  return (
    <div className="bg-[#0a050f] text-[#f5f3ff] min-h-screen font-serif antialiased">
      {/* NAVBAR */}
      <nav className="fixed top-0 inset-x-0 z-50 p-8 flex justify-between items-center bg-[#0a050f]/60 backdrop-blur-xl border-b border-[#a855f7]/5">
      ...
      <section className="relative h-screen flex items-center justify-center pt-24 overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0a050f]/80 via-[#0a050f]/60 to-[#0a050f]" />
      ...
      <section id="treatments" className="py-32 px-8 max-w-7xl mx-auto">
        <div className="text-center mb-28">
           <h2 className="text-xs tracking-[0.8em] text-[#a855f7] mb-6 uppercase italic font-bold">OUR TREATMENTS</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
          {services.map((s) => (
            <div key={s.id} className="group relative cursor-pointer bg-white/[0.02] p-12 overflow-hidden rounded-[3rem] border border-white/5 hover:border-[#a855f7]/40 hover:bg-[#a855f7]/5 transition-all shadow-2xl" onClick={() => openBooking(s)}>
              <div className="absolute top-0 right-0 p-8">
                 <span className="bg-[#a855f7] text-white px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest uppercase shadow-lg shadow-[#a855f7]/30 group-hover:scale-110 transition-transform">{s.durationMinutes} MIN</span>
              </div>
              <h3 className="text-2xl font-black text-white mb-6 group-hover:text-[#a855f7] transition-all uppercase tracking-tight italic pt-4">{s.name}</h3>
              ...
              <div className="text-3xl font-black text-[#a855f7]">€{s.price}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* HOURS & INFO */}
      <section className="bg-[#0f0a1f] py-40 px-8 border-y border-white/5">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-24 items-center">
          <div className="space-y-12">
             <div className="text-center md:text-left">
                <h2 className="text-[#a855f7] text-[10px] tracking-[0.5em] uppercase italic font-black mb-8 underline underline-offset-8">THE SALON</h2>
                <p className="text-2xl font-bold leading-relaxed pr-8">{business.address}</p>
                <div className="mt-8 flex gap-4 justify-center md:justify-start">
                   {business.socialLinks?.instagram && <a href={business.socialLinks.instagram} className="p-4 bg-white/5 rounded-full hover:bg-[#a855f7] hover:text-white transition-all"><Instagram size={18}/></a>}
                   {business.socialLinks?.facebook && <a href={business.socialLinks.facebook} className="p-4 bg-white/5 rounded-full hover:bg-[#a855f7] hover:text-white transition-all"><Facebook size={18}/></a>}
                </div>
             </div>
          </div>

          <div className="text-center relative">
             <div className="absolute -inset-10 bg-[#a855f7]/5 blur-3xl rounded-full" />
             <h2 className="text-white text-5xl font-black italic tracking-tighter uppercase mb-6">{business.name}</h2>
             <p className="text-[#a855f7] text-[10px] tracking-[0.5em] font-black uppercase">RESERVE: {business.phone}</p>
          </div>
          ...
          <div className="space-y-6">
             {hours.sort((a,b) => a.dayOfWeek - b.dayOfWeek).map((h) => (
                <div key={h.dayOfWeek} className="flex justify-between items-center py-4 border-b border-white/5 group">
                  <span className="text-[#a855f7] uppercase tracking-widest text-[10px] font-black opacity-40 group-hover:opacity-100 transition-opacity">{['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][h.dayOfWeek]}</span>
      ...
      <footer className="p-16 text-center italic text-white/10 font-bold uppercase tracking-[1em] text-[8px] bg-[#0a050f]">
