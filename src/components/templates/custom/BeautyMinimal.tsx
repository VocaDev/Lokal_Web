...
export default function BeautyMinimal({ business, services, hours }: {
  ...
  return (
    <div className="bg-[#050505] text-[#f472b6]/80 min-h-screen font-light antialiased">
      {/* NAVBAR */}
      ...
      <section id="treatments" className="py-32 px-8 max-w-5xl mx-auto">
        <h2 className="text-[10px] tracking-[0.8em] text-[#f472b6]/30 mb-20 uppercase font-black text-center">TREATMENTS</h2>
          ...
          {services.map((s) => (
            <div key={s.id} className="flex justify-between items-center py-10 border-b border-white/5 group cursor-pointer hover:bg-white/[0.01] px-4 transition-all" onClick={() => openBooking(s)}>
              <div className="space-y-2">
                <h3 className="text-xl text-white tracking-widest font-black uppercase group-hover:text-[#f472b6] transition-colors">{s.name}</h3>
                <div className="flex gap-4 items-center">
                   <div className="w-8 h-px bg-[#f472b6]/20" />
                   <span className="text-[10px] tracking-widest uppercase text-[#f472b6]/40">{s.durationMinutes} MIN</span>
                </div>
              </div>
      ...
      <section className="py-40 px-8 max-w-5xl mx-auto grid md:grid-cols-2 gap-32 border-t border-white/5">
        <div>
           <div className="flex items-center gap-4 mb-20">
              <div className="h-10 w-10 flex items-center justify-center rounded-full border border-[#f472b6]/20 text-[#f472b6]"><Sparkles size={18}/></div>
              <h2 className="text-lg font-black text-white tracking-[0.3em] uppercase underline underline-offset-[12px] decoration-[#f472b6]/20">VISIT</h2>
           </div>
           ...
           <div className="flex gap-6">
              {business.socialLinks?.instagram && <a href={business.socialLinks.instagram} className="text-[#f472b6]/40 hover:text-[#f472b6] transition-colors"><Instagram size={20}/></a>}
              {business.socialLinks?.facebook && <a href={business.socialLinks.facebook} className="text-[#f472b6]/40 hover:text-[#f472b6] transition-colors"><Facebook size={20}/></a>}
           </div>
        </div>

        <div>
           <h2 className="text-[10px] tracking-[0.5em] text-[#f472b6]/30 mb-16 uppercase font-black">OPENING HOURS</h2>
           <div className="space-y-6">
              {hours.sort((a,b) => a.dayOfWeek - b.dayOfWeek).map((h) => (
                <div key={h.dayOfWeek} className="flex justify-between items-baseline group">
                  <span className="text-[10px] tracking-[0.3em] text-[#f472b6]/20 uppercase font-black group-hover:text-[#f472b6]/40 transition-colors">{['Sun','Mon','Tue','Wed','Thu','Fri','Sat'][h.dayOfWeek]}</span>
      ...
      <footer className="p-20 text-center border-t border-white/5 bg-[#080808]">
