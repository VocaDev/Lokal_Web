      ...
      <section id="treatments" className="py-32 px-8 max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-baseline mb-20 gap-8">
           <div>
              ...
              <div className="w-12 h-2 bg-primary mt-4" />
           </div>
        ...
        <div className="grid md:grid-cols-2 gap-10">
          {services.map((s) => (
            <div key={s.id} className="group relative cursor-pointer bg-slate-900/50 p-10 rounded-[3rem] border border-white/5 hover:bg-primary/10 hover:border-primary/20 transition-all" onClick={() => openBooking(s)}>
              ...
              <h3 className="text-2xl font-black text-white mb-4 pr-16 group-hover:text-primary transition-colors uppercase tracking-tight italic">{s.name}</h3>
              ...
              <div className="text-[10px] font-black text-primary tracking-widest uppercase py-2 border-t border-white/5 inline-block">
          ...
          <div>
            ...
            <div className="space-y-8">
               <div className="flex items-center gap-6 group">
                  <div className="bg-primary/10 p-5 rounded-3xl text-primary group-hover:bg-primary group-hover:text-white transition-all"><MapPin /></div>
                  <span className="text-xl font-bold">{business.address}</span>
               </div>
               <div className="flex items-center gap-6 group">
                  <div className="bg-primary/10 p-5 rounded-3xl text-primary group-hover:bg-primary group-hover:text-white transition-all"><Phone /></div>
          ...
          <div>
             ...
             <div className="grid grid-cols-2 gap-4">
                {hours.sort((a,b) => a.dayOfWeek - b.dayOfWeek).map((h) => (
                  <div key={h.dayOfWeek} className="p-6 bg-background rounded-3xl border border-white/5">
                    <span className="text-primary uppercase tracking-widest text-[10px] font-black block mb-2">{['SUN','MON','TUE','WED','THU','FRI','SAT'][h.dayOfWeek]}</span>
