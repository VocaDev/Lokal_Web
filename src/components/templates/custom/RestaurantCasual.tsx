...
export default function RestaurantCasual({ business, services, hours }: {
  ...
  return (
    <div className="bg-zinc-950 text-zinc-100 min-h-screen font-sans">
      {/* NAVBAR */}
      <nav className="fixed top-4 inset-x-4 z-50 px-4 py-3 flex justify-between items-center bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <Utensils className="h-5 w-5 text-red-500 shrink-0" />
          <span className="font-black tracking-tight text-base sm:text-lg italic uppercase truncate">{business.name}</span>
        </div>
        <button onClick={() => openBooking()} className="shrink-0 min-h-[44px] inline-flex items-center bg-red-600 text-white px-4 sm:px-6 py-3 rounded-xl text-xs font-bold tracking-widest uppercase hover:bg-red-700 transition-colors shadow-lg shadow-red-600/20">
          {(business.ctaPrimary || 'BOOK A TABLE').toUpperCase()}
        </button>
      </nav>

      {/* HERO */}
      <section className="relative min-h-[80svh] md:min-h-screen flex items-center justify-center pt-24 px-4 md:px-8 text-center bg-gradient-to-b from-zinc-900 to-zinc-950 overflow-hidden">
        {heroImg && (
          <>
        ...
        <div className="relative z-10 max-w-3xl">
          ...
          <h1 className="text-5xl sm:text-6xl md:text-8xl font-black italic tracking-tighter mb-8 uppercase leading-[0.9]">
            {business.heroHeadline || business.name}
          </h1>
          <p className="text-zinc-400 text-base sm:text-lg md:text-xl leading-relaxed mb-12 max-w-xl mx-auto">
          ...
          <button onClick={() => document.getElementById('menu')?.scrollIntoView({behavior: 'smooth'})} className="min-h-[44px] inline-flex items-center bg-zinc-100 text-zinc-950 px-7 sm:px-10 py-3 sm:py-4 rounded-xl text-sm font-black tracking-widest uppercase hover:scale-105 transition-all shadow-xl shadow-white/5">
      ...
      {/* MENU */}
      <section id="menu" className="py-20 md:py-32 px-4 md:px-8 max-w-6xl mx-auto">
        <div className="mb-12 md:mb-20">
          <h2 className="text-4xl sm:text-5xl font-black italic tracking-tighter uppercase mb-2">WHAT WE SERVE</h2>
          <div className="w-20 h-2 bg-red-600" />
        </div>

        <div className="grid md:grid-cols-2 gap-6 md:gap-10">
          {services.map((s) => (
            <div key={s.id} className="group cursor-pointer bg-zinc-900 p-6 md:p-8 rounded-2xl border-l-4 border-red-600 hover:bg-zinc-800 transition-colors" onClick={() => openBooking(s)}>
      ...
      {/* INFO */}
      <section className="bg-zinc-900 py-20 md:py-32 px-4 md:px-8 border-y border-zinc-800">
      ...
      <footer className="px-4 py-10 md:p-12 text-center text-zinc-600 font-bold uppercase tracking-[0.2em] text-[10px]">
