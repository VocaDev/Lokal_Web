...
export default function RestaurantElegant({ business, services, hours }: {
  ...
  return (
    <div className="bg-[#0f0a05] text-[#fef3c7] min-h-screen font-serif">
      {/* NAVBAR */}
      ...
      <section id="menu" className="py-32 px-8 max-w-5xl mx-auto">
        <div className="text-center mb-24">
          <h2 className="text-xs tracking-[0.6em] text-[#d97706] mb-4 uppercase">OUR MENU</h2>
        ...
        <div className="grid md:grid-cols-2 gap-x-20 gap-y-16">
          {services.map((s) => (
            <div key={s.id} className="relative group cursor-pointer" onClick={() => openBooking(s)}>
              <div className="flex justify-between items-baseline mb-2 border-b border-[#d97706]/20 pb-2">
                <h3 className="text-xl font-bold group-hover:text-[#d97706] transition-colors uppercase">{s.name}</h3>
                <span className="text-xl font-medium">€{s.price}</span>
              </div>
              <p className="text-[#d97706]/50 text-sm leading-relaxed italic">{s.description || "A signature creation prepared by our chef."}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* HOURS & INFO */}
      <section className="bg-[#1a120b] py-32 px-8">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-16">
          <div className="text-center md:text-left">
            <h4 className="text-[#d97706] text-xs tracking-widest uppercase mb-8">Location</h4>
            <p className="text-lg leading-relaxed">{business.address}</p>
            <div className="mt-8 flex gap-4 justify-center md:justify-start">
              <a href={`tel:${business.phone}`} className="p-3 border border-[#d97706]/20 rounded-full hover:bg-[#d97706]/10 transition-colors"><Phone size={16}/></a>
            </div>
          </div>
          
          <div className="text-center">
            <h4 className="text-[#d97706] text-xs tracking-widest uppercase mb-8">Service Hours</h4>
            <div className="space-y-4">
              {hours.sort((a,b) => a.dayOfWeek - b.dayOfWeek).map((h) => (
                <div key={h.dayOfWeek} className="flex justify-between text-sm italic">
                  <span className="text-[#d97706]/60 uppercase tracking-widest">{['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][h.dayOfWeek]}</span>
          ...
          <div className="text-center md:text-right">
            <h4 className="text-[#d97706] text-xs tracking-widest uppercase mb-8">Contact</h4>
            <p className="text-2xl font-bold mb-4">{business.phone}</p>
            <p className="text-[#d97706]/60 text-sm italic">For private events and inquiries.</p>
          </div>
        </div>
      </section>

      <footer className="p-12 text-center border-t border-[#d97706]/10">
        <p className="text-[10px] tracking-[0.4em] text-[#d97706]/40 uppercase">© 2026 {business.name} — Designed by LokalWeb</p>
      </footer>
