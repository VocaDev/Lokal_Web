/**
 * Stage 2: Theme Generator (parametric AI renderer) — pure runner.
 *
 * Lives outside app/api/generate-variants/route.ts because Next.js's App
 * Router validates route file exports against a fixed allowlist (GET/POST/etc.,
 * runtime, dynamic, etc.). Helper exports like generateTheme or
 * GenerateThemeArgs cause the build to fail with "X is not a valid Route
 * export field."
 *
 * The route's POST handler imports generateTheme from here and wraps it with
 * auth + rate limiting + post-processing + validation + retry. The scripts/
 * A/B harness imports it directly to bypass the Next runtime entirely.
 *
 * Anything in this file is "the prompt-building + raw model call." The
 * post-processor and validator stay in the route since they're tied to the
 * production HTTP shape.
 */
...
Generate the theme.`;
  ...
  // Haiku vs Sonnet actually wrote.
  try {
